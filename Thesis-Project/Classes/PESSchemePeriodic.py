...
class PESSchemePeriodic(PESScheme):
    ...
    def start_sale(self, asset_id: int, pes_goal: int, number_of_payments: int):        
        print("Entrei aqui")
        ...
        bytes_args = [START_SALE.encode(), pes_goal, number_of_payments]
...
def validate_json(config_json: dict) -> bool:
    if "nft_name" not in config_json.keys():
        ...
        return False
    elif "number_of_payments" not in config_json.keys():
