...
def approval():
    ...
    # GLOBAL - App Create
    #Minimum-Balance = 100000 + (28500) * 8 + (50000) * 2 = 100000 + 228000 + 100000 = 428000 = 4,28 * MIN_BALANCE
    ...
    # Global Int
    global_fraction_asset_id = Bytes("Fractional_Asset_Id")
    ...
    # Global Int
    global_nft_price = Bytes("NFT_Price")
    ...
    def hasSaleNotStarted():
        return Seq(
            Assert(
                And(
                    App.globalGet(global_fraction_asset_id) == Int(0),
    ...
    def isWaitingForResponse():
        return Seq(
            Assert(
                And(
                    App.globalGet(global_fraction_asset_id) != Int(0),
                    App.globalGet(global_state) == WaitingForResponse,
                )
            )
        )
    ...
    def hadNegativeResponse():
        return Seq(
            Assert(
                And(
                    App.globalGet(global_fraction_asset_id) != Int(0),
                    App.globalGet(global_state) == NegativeResponse,
                )
            )
        )
    ...
    def hadPositiveResponse():
        return Seq(
            Assert(
                And(
                    App.globalGet(global_fraction_asset_id) != Int(0),
                    App.globalGet(global_state) == PositiveResponse,
                )
            )
        )
    ...
    nft_seller_init = Seq(
        ...
        # Initializes the Fractional Value
        App.globalPut(global_fraction_asset_id, Int(0)),
        # Approves  
        Approve(),
    )
    ...
    # 0. Start_Sale
    # 1. The name of NFTs to be created
    # 2. The unit_name of NFTs to be created
    # 3. The decimals of NFTs to be created
    # 4. The nft_price
    # 5. The period
    ...
    start_sale = Seq(
        ...
        InnerTxnBuilder.Submit(),
        # Creates an Inner Transaction to create the fraction NFTs
        InnerTxnBuilder.Begin(),
        # Creates AssetConfig Transaction
        InnerTxnBuilder.SetFields(
            {
                ...
                TxnField.config_asset_total: Exp(Int(10), Btoi(Txn.application_args[3])),
        ...
        # Stores the created Id's Asset
        App.globalPut(global_fraction_asset_id, InnerTxn.created_asset_id()),
        # Starts the sale
        App.globalPut(global_state, WaitingForResponse),
        # Stores the nft price
        App.globalPut(global_nft_price, Btoi(Txn.application_args[4])),
        # Stores the period
        App.globalPut(global_receive_timestamp, Add(Global.latest_timestamp(), Mul(Int(60), Btoi(Txn.application_args[5])))),
        # Approves
        Approve(),
    )

    asset_balance = AssetHolding.balance(Global.current_application_address(), App.globalGet(global_fraction_asset_id))

    # Called in the No_Op
    # The buy_NFT function receives a group of arguments:
    # 0. buy_NFT
    # 1. The quantity of NFT bought
    # The buy_NFT function receives a group of transactions:
    ...
    buy_NFT = Seq(
        ...
        defaultTransactionChecks(Int(0)),
        defaultTransactionChecks(Int(1)),
        # Verifies if the NFT seller can fullfill the necessary quantity
        # Verifies if the NFT Seller has the NFT
        asset_balance,
        # Assert(AssetHolding.balance(
        #    Global.current_application_address(), App.globalGet(global_fraction_asset_id)).hasValue()),
        Assert(asset_balance.hasValue()),
        # Verifies if the NFT Seller has the request quantity of the NFT
        Assert(asset_balance.value() >= Btoi(Gtxn[1].application_args[1])),
        # Verifies if the payment is enough for the request quantity of NFT
        Assert(Mul(App.globalGet(global_nft_price),
               Btoi(Gtxn[1].application_args[1])) <= Gtxn[0].amount()),
        # Loads box
        ...
        # Creates an InnerTransactions to send to token to the buyer
        InnerTxnBuilder.Begin(),
        InnerTxnBuilder.SetFields(
            {
                TxnField.type_enum: TxnType.AssetTransfer,
                TxnField.asset_receiver: Gtxn[0].sender(),
                TxnField.xfer_asset: App.globalGet(global_fraction_asset_id),
                TxnField.asset_amount: Btoi(Gtxn[1].application_args[1]),
                TxnField.fee: Int(0),
            }
        ),
        # Continues a group transaction
        InnerTxnBuilder.Next(),
        # Freezes the Asset
        InnerTxnBuilder.SetFields(
            {
                ...
                TxnField.sender: Global.current_application_address(),
                TxnField.freeze_asset: App.globalGet(global_fraction_asset_id),
                TxnField.freeze_asset_account: Gtxn[0].sender(),
    ...
    return program.event(
        ...
        no_op=Cond(
            ...
            [Txn.application_args[0] == op_buy_NFT, buy_NFT],
