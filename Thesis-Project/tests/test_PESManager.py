...
from Classes.SmartContract import var_to_base64, MIN_BALANCE, LOCAL_BYTE_PRICE, LOCAL_INT_PRICE, GLOBAL_BYTE_PRICE, GLOBAL_INT_PRICE
...
@pytest.fixture
def setup_environment(setup_algod_client, deploy_land_registry, setup_account, setup_account2, setup_pinata):
    # Loads Land Registry
...
def test_PES_Manager_deployment(setup_environment, setup_account4, deploy_oracle, setup_algod_client):
    # Loads oracle app id
    oracle_app_id = pytest.oracle_app_id
    # Old Balance
    old_balance = setup_account4.get_balance()
    # Creates an PES Manager
    pes_manager = PESManager(setup_account4, 0, oracle_app_id, pytest.land_registry_address)
    # New balance
...
def test_PES_Manager_opt_in(setup_account, setup_algod_client):
    ...
    assert -balance_diff == 2500 + 400 * (len(decode_address(setup_account.address)) + 1) + 2 * sp.min_fee
...
def test_create_PES_Scheme(setup_account2, setup_algod_client):
    ...
    pes_scheme = pes_manager_user.create_PES_scheme()
...
def test_start_sale_PES_Scheme(setup_account2, setup_pinata, setup_algod_client):
    ...
    # Obtains info from the oracle
    oracle_account_info = algod_client.account_application_info(pes_scheme.app_address ,oracle_app_id)
    oracle_local_variables = get_local_variables(oracle_account_info)
    # Verifies if the app id has been saved
    assert oracle_local_variables['local_app_id'] == pes_scheme.app_id
    assert oracle_local_variables['local_state'] == 1
    # Sleeps for 10 seconds
    # sleep(10)
    request_num = len(oracle.request_list)
    while request_num == len(oracle.request_list):
        oracle.get_requests(start_time, end_time)
        end_time = rfc3339_step()
    # Gets request
    # oracle.get_requests(start_time, end_time)
    ...
    sp = setup_algod_client.suggested_params()
    print(balance_diff)
    assert balance_diff == 12 * sp.min_fee + 4 * MIN_BALANCE + 2 * LOCAL_INT_PRICE
...
def test_buy_PESScheme(setup_algod_client, setup_account4):
    ...
    old_balance = setup_account4.get_balance()
    # Buys the NFT
    pes_scheme_user = PESSchemeUser(setup_account4, pytest.pes_scheme.app_id)
    quantity = 5
    ...
    sp = setup_algod_client.suggested_params()
    print(balance_diff)
    assert balance_diff == (5 * 50) + 7 * sp.min_fee  + (2500 + 400 * (32 + 8))

def test_buy_PESScheme2(setup_algod_client, setup_account5):
    # See the state of the seller and the asset before the buy
    seller_asset_info = (setup_algod_client.account_asset_info(get_app_address(pytest.pes_scheme.app_id), pytest.fractional_asset_id))['asset-holding']
    asset_info = setup_algod_client.asset_info(pytest.fractional_asset_id)
    assert seller_asset_info['amount'] != asset_info['params']['total']
    ...
    # Buys the Asset
    pes_scheme_user = PESSchemeUser(setup_account5, pytest.pes_scheme.app_id)
    quantity = 5
    # Checks account balance before buy
    # Loads the Period
...
def test_show_created_pes_scheme(setup_account2, setup_algod_client):
    ...
    pes_manager_user.create_PES_scheme()
    pes_manager_user.create_PES_scheme()
    pytest.pes_scheme2 =  pes_manager_user.create_PES_scheme()
...
def test_give_refund2(setup_account5, setup_algod_client):
    ...
    old_balance = setup_account5.get_balance()
    # Gives refund
    pes_scheme.give_refund(setup_account5.address)
    # Checks account balance before refund
    new_balance = setup_account5.get_balance()
    balance_diff = new_balance - old_balance
    print(balance_diff)
