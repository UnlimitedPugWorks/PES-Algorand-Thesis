...
def test_start_sale_PES_Scheme(setup_account2, setup_pinata, setup_algod_client):
    ...
    created_asset_id = pes_scheme.start_sale(asset_id = sold_asset_id ,file_path = "pes_config2.json")
...
def test_get_refund(setup_account4, setup_algod_client):
    ...
    print(old_balance)
    # Creates NFT_Buyer
    nft_buyer = PESSchemeAssetUser(setup_account4, pytest.pes_scheme.app_id)
    ...
    assert balance_diff == (100000 * 5) + box_cost(decode_address(setup_account4.address), 8) - 2 * sp.min_fee
    

def test_get_refund2(setup_account6, setup_algod_client):
    # Loads balance before refund
    old_balance = setup_account6.get_balance()
    print(old_balance)
    # Creates NFT_Buyer
    nft_buyer = PESSchemeAssetUser(setup_account6, pytest.pes_scheme.app_id)
    nft_buyer.get_refund()
    # Gets sp
    sp = setup_algod_client.suggested_params()
    ...
    assert new_balance > old_balance
    #assert balance_diff == ((100000 * 5) + (2500 + 400 * (32 + 8)) - 2 * sp.min_fee )
    assert balance_diff == (100000 * 5) + box_cost(decode_address(setup_account6.address), 8) - 2 * sp.min_fee
...
def test_final_costs(setup_algod_client, setup_account, setup_account2, setup_account3, setup_account4, setup_account5, setup_account6):
    ...
    # 341700 = 438200
    assert abs(pytest.balance2 - balance2) + abs(pytest.min_balance2 - min_balance2) == 3 * MIN_BALANCE  + box_cost(decode_address(setup_account2.address), 1) + 26 * sp.min_fee
    ...
    assert abs(pytest.balance4 - balance4) + abs(pytest.min_balance4 - min_balance4) == 9 * sp.min_fee + MIN_BALANCE
    print("BUYER 2")
    ...
    assert abs(pytest.balance6 - balance6) + abs(pytest.min_balance6 - min_balance6) == 9 * sp.min_fee + MIN_BALANCE
