...
def test_get_refund2(setup_account6, setup_algod_client):
    ...
    assert new_balance > old_balance
    #assert balance_diff == ((100000 * 5) + (2500 + 400 * (32 + 8)) - 2 * sp.min_fee )
    assert balance_diff == box_cost(decode_address(setup_account6.address), 8) - 2 * sp.min_fee
...
def test_final_costs(setup_algod_client, setup_account, setup_account2, setup_account3, setup_account4, setup_account5, setup_account6):
    ...
    # 335200 = 341200
    assert abs(pytest.balance2 - balance2) + abs(pytest.min_balance2 - min_balance2) == 3 * MIN_BALANCE  + box_cost(decode_address(setup_account2.address), 1) + 20 * sp.min_fee -  (10 * pytest.nft_price)
    ...
    assert abs(pytest.balance4 - balance4) + abs(pytest.min_balance4 - min_balance4) == (5 * pytest.nft_price) + 9 * sp.min_fee + MIN_BALANCE
    print("BUYER 2")
    ...
    assert abs(pytest.balance6 - balance6) + abs(pytest.min_balance6 - min_balance6) == (5 * pytest.nft_price) + 9 * sp.min_fee + MIN_BALANCE
