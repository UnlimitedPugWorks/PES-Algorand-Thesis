...
def test_create_PES_Scheme(setup_account2, setup_algod_client):
    ...
    pes_scheme = pes_manager_user.create_PES_scheme(type=0)
    ...
    sp = setup_algod_client.suggested_params()
    print(balance_diff)
    assert balance_diff == deploy_cost(7, 2) + 3 * sp.min_fee
    algod_client = setup_algod_client
...
def test_start_sale_PES_Scheme(setup_account2, setup_algod_client):
    ...
    # Starts the sale
    pes_scheme.start_sale(asset_id = sold_asset_id , pes_goal = 500, period = period)
    algod_client = setup_algod_client
    app_info = algod_client.application_info(pes_scheme.app_id)
    global_vars = get_global_variable(app_info)
    # print(global_vars)
...
def test_buy_PESScheme(setup_algod_client, setup_account4):
    ...
    # Buys the NFT
    pes_scheme_user = PESSchemeNoAssetUser(setup_account4, pytest.pes_scheme.app_id)
    quantity = 250
    ...
    sp = setup_algod_client.suggested_params()
    print(balance_diff)
    assert balance_diff == (quantity) + 4 * sp.min_fee  + box_cost(decode_address(setup_account4.address), 8)
...
def test_buy_PESScheme2(setup_algod_client, setup_account6):
    ...
    # Buys the Asset
    pes_scheme_user = PESSchemeNoAssetUser(setup_account6, pytest.pes_scheme.app_id)
    quantity = 250
    pes_scheme_user.buy_NFT(quantity)
    ...
    sp = setup_algod_client.suggested_params()
    print(balance_diff)
    assert balance_diff == (quantity) + 4 * sp.min_fee  + box_cost(decode_address(setup_account6.address), 8)
...
def test_show_created_pes_scheme(setup_account2, setup_algod_client):
    ...
    #pes_manager_user.create_PES_scheme()
    #pes_manager_user.create_PES_scheme()
    pytest.pes_scheme2 =  pes_manager_user.create_PES_scheme(type=0)
...
def test_give_refund(setup_account4, setup_account2, setup_algod_client):
    ...
    print(balance_diff)
    sp = setup_algod_client.suggested_params()
    assert balance_diff == box_cost(decode_address(setup_account4.address), 8) + 250
...
def test_give_refund2(setup_account6, setup_account2, setup_algod_client):
    ...
    print(balance_diff)
    sp = setup_algod_client.suggested_params()
    assert balance_diff ==  box_cost(decode_address(setup_account6.address), 8) + 250
    assert balance_diff2 == -2 * sp.min_fee
...
def test_final_costs(setup_algod_client, setup_account, setup_account2, setup_account3, setup_account4, setup_account5, setup_account6):
    ...
    # 244700 = 344700
    assert abs(pytest.balance2 - balance2) + abs(pytest.min_balance2 - min_balance2) == 2 * MIN_BALANCE  + box_cost(decode_address(setup_account2.address), 1) + 29 * sp.min_fee
    ...
    assert abs(pytest.balance3 - balance3) + abs(pytest.min_balance3 - min_balance3) == 3 * MIN_BALANCE + 2 * sp.min_fee + deploy_cost(1, 2)
    print("BUYER 1")
    ...
    assert abs(pytest.balance4 - balance4) + abs(pytest.min_balance4 - min_balance4) == 4 * sp.min_fee
    print("BUYER 2")
    ...
    assert abs(pytest.balance6 - balance6) + abs(pytest.min_balance6 - min_balance6) == 4 * sp.min_fee
